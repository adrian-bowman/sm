...
#      if (any(is.na(nseg[[i]])) | prod(nseg[[i]]) > 400)
   ...
   if (!random.missing)  result$sigma.r <- sigr
   class(result) <- "pam"
...
plot.pam <- function(model, components = 1:length(model$xlabels), plotinfo, 
   ...
   replace.na(opt, include.mean,            opt$include.lower.terms)
   ...
   if (requireNamespace("colorspace", quietly = TRUE)) 
      col.pal <- if (opt$include.lower.terms) heat_hcl(100) else diverge_hcl(100)
   ...
   if (missing(plotinfo)) {
      ...
      for (i in components) {
         ...
         if (ngrid.missing) ngrid <- switch(ndim, 100, 20, 10)
         u   <- list(length = ndim)
         for (j in 1:ndim)
         ...
         if (se) {
            ...
            if (opt$reference == "no effect")
               result$reference <- est / array(result$st.error, dim(est))
         }
   ...
   if (length(ndim1) > 0) {
      ...
      for (i in 1:length(ndim1)) {
      	 ...
      	 xx        <- c(pinf$x)
         x.add     <- c(xx, uu)
         y.add     <- c(model$y - model.fitted + pinf$mui, pinf$est)
         u.all     <- c(u.all, uu)
         est.all   <- c(est.all, pinf$est)
         lblx.add  <- c(rep("data", length(xx)), rep("line", length(uu)))
         if (se) {
            se1.all   <- c(se1.all,  pinf$est + 2 * pinf$st.error)
            se2.all   <- c(se2.all,  pinf$est - 2 * pinf$st.error)
         	x.add     <- c(x.add, uu, rev(uu))
         	y.add     <- c(y.add, pinf$est + 2 * pinf$st.error, rev(pinf$est - 2 * pinf$st.error))
   	  ...
   	  if (se) {
   	  	 dfrm2 <- data.frame(x = c(u.all, rev(u.all)), y = c(se1.all, rev(se2.all)))
   	  ...
   	  plt   <- plt + ggplot2::geom_line(data = subset(dfrm, lbl.x == "line"), col = "blue")
      ...
      for (i in ndim2) {
         ...
         if (ndim == 2) {
            ...
            if (display %in% c("persp", "lines")) 
               ...
               # image(u[[2]], u[[1]], t(est * mask), zlim = ylim, col = opt$col.palette, add = TRUE)
               ...
               layout(1)
   	           ...
   	           # dfrm <- data.frame(x = rep(u[[opt$order[2]]], length(u[[opt$order[1]]])),
   	                              # y = rep(u[[opt$order[1]]], each = length(u[[opt$order[2]]])),
   	                              # z = c(t(aperm(est * mask, opt$order[1:2]))))
   	           # dfrm <- subset(dfrm, !is.na(x + y + z))
    	       # if (requireNamespace("akima", quietly = TRUE)) {
  	              # grdx   <- seq(min(dfrm$x), max(dfrm$x), length = 100)
  	              # grdy   <- seq(min(dfrm$y), max(dfrm$y), length = 100)
  	              # intrp <- akima::interp(dfrm$x, dfrm$y, dfrm$z, grdx, grdy)
  	              # dfrm <- data.frame(x = rep(intrp$x, length(intrp$y)),
  	                                 # y = rep(intrp$y, each = length(intrp$x)),
  	                                 # z = c(intrp$z))
  	           # }
   	           # ind  <- !is.na(apply(as.matrix(dfrm), 1, sum))
   	           # dfrm <- dfrm[ind, ]
               # plt  <- ggplot2::ggplot(dfrm, ggplot2::aes(x, y, z = z)) +
                       # ggplot2::geom_tile(ggplot2::aes(fill = z))
               # if (!opt$include.lower.terms)
                  # plt <- plt + ggplot2::scale_fill_gradient2(low = scales::muted("blue"),
                                                             # high = scales::muted("red"))
               # else
                  # plt <- plt + ggplot2::scale_fill_gradient()
               # plt <- plt + ggplot2::xlab(xlab[opt$order[2]]) + ggplot2::ylab(xlab[opt$order[1]])
                             # # ggplot2::geom_point(ggplot2::aes(colour = z))

               # if (se) {
                  # refer <- if (opt$reference == "no effect") reference
                           # else aperm(array(st.error, dim = dim(est)), opt$order[1:2])
   	              # z     <- c(t(aperm(refer * mask, opt$order[1:2])))
                  # dfrm1 <- data.frame(x = rep(u[[opt$order[2]]], length(u[[opt$order[1]]])),
   	                                  # y = rep(u[[opt$order[1]]], each = length(u[[opt$order[2]]])), z = z)
   	              # ind   <- !is.na(apply(as.matrix(dfrm1), 1, sum))
   	              # dfrm1 <- dfrm1[ind, ]
   	              # z     <- z[ind]
   	              # brks  <- NULL
   	              # if (opt$reference == "no effect") {
   	                 # if (min(z) <= -2) brks <- c(brks, seq(floor(min(z)), -2))
   	                 # if (max(z) >=  2) brks <- c(brks, seq(2, ceiling(max(z))))
   	              # }
                  # plt   <- plt + ggplot2::stat_contour(data = dfrm1, ggplot2::aes(colour = ..level..),
                                    # breaks = brks) +
                                 # ggplot2::scale_colour_gradient2()

                  # # contour(u[[2]], u[[1]], t(reference * mask), add = TRUE, lty = 2)
               # }
               # print(plt)
            ...
            if (opt$reference == "no effect") mdl$reference <- reference
            fg.plot <- if (is.function(opt$foreground.plot)) opt$foreground.plot else I
            print(xdims)
            print(xlab)
            print(opt$order[3])
            rp.plot4d(x[ , opt$order[1:2]], x[ , opt$order[3]], mui, mdl, ylab = ylab,
